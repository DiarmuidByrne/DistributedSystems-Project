	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(req.getParameter("frmMaxKeyLength") != null) {			
			...
			if (taskNumber == null){
				...
				// If user inputs a different ip, the request will be sent to that address instead 
				if(hostIP.length() > 0) { 
					Request newReq = new Request(cypherText, maxKeyLength, Integer.parseInt(taskNumber.substring(1)), hostIP);
					vrm.add(newReq);
				// Else, run the RMI locally
				} else {
					Request newReq = new Request(cypherText, maxKeyLength, Integer.parseInt(taskNumber.substring(1)));		
					vrm.add(newReq);
				}
